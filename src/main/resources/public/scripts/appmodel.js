var appModel = (function () {
    ...
    var privateNewGame = function (type) {
      return $.post('/haresandhounds/api/games', JSON.stringify({pieceType : type}), null, 'json')
        .done(function (data) {
    ...
    var privateJoinGame = function (gameId) {
      return $.ajax({
         url : '/haresandhounds/api/games/' + gameId,
         method : "PUT",
         dataType : 'json'
      })
        .done(function (data) {
    ...
    var privateFetchState = function() {
        return $.get('/haresandhounds/api/games/' + state.gameId + '/state', "", null, 'json')
            .done(function (data) {
    ...
    var privateFetchBoard = function() {
        return $.get('/haresandhounds/api/games/' + state.gameId + '/board', "", null, 'json')
            .fail(function (jqXHR) {
               console.log('Error ' + jqXHR.status);
            });
    };
    ...
    var privateMovePiece = function(from, to) {
        ...
        return $.post('/haresandhounds/api/games/' + state.gameId + "/turns", JSON.stringify(postData))
            .fail(function (jqXHR) {
                //Todo
                console.log('Error ' + jqXHR.status);
            });
